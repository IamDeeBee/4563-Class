...
//***************************************************************
// File:     BlockCyclicPartitionExamplec
// Purpose:  Use MPI to perform a Block Cyclic partition of an array of size 20
...
void rearrangeArray(int *,int ,int*,int,int,int);


int main(int argc, char **argv){
	//For all participant processes
	int *orig_array, *blockCyclic_array, size, subBlockSize;
	...
	if(rank==0){
		...
		rearrangeArray(orig_array,size,cyclic_array,local_size,comm_sz,subBlockSize);
		free(orig_array);
	}
...
Parameter:
  ...
  int subsize: used to rearrange the array among processes
  int comm_sz: number of processes
  int blockDivide: used to rearrange the array among sub Blocks
  
Returns:
  void
*/
void rearrangeArray(int *old_array,int size,int *new_array,int subsize,int commsz, int blockDivide){
	int a = 0;
  int b = 1;
  int c;
  int jump = ((comm_sz-1)*blockDivide)+1;

  for(int i = 0; i < size; i++){
    new_array[i] = old_array[a];
    if((i+1)%blockDivide!=0){
      a = a + 1;
    }else{
      a = a + jump;
    }
